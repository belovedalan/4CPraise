      ...
      playerRef.current = new window.YT.Player(playerHostRef.current, {
        ...
        events: {
          ...
          onStateChange: (event: any) => { 
            ...
            if (event.data === 1) setTimeout(() => setVideoReady(true), 150);
      ...
      <main className="page" onClick={() => {
        ...
        <style>{`
          ...
          .glass-card { 
            ...
            box-shadow: 0 4px 24px -1px rgba(0, 0, 0, 0.05), inset 0 0 0 1px rgba(255, 255, 255, 0.4);
            overflow: hidden;
          }

          /* ✅ 播放卡片：底部极淡的流光，只作为视觉呼吸点 */
          .glass-hero::after {
            content: ""; position: absolute; inset: 0; pointer-events: none;
            background: radial-gradient(ellipse at 50% 110%, rgba(124, 58, 237, 0.12) 0%, transparent 70%);
          }
          ...
          .progressBarContainer { width: 100%; height: 1px; background: rgba(0,0,0,0.03); position: relative; }
          .progressBarActive { height: 100%; background: var(--accent); opacity: 0.8; transition: width 0.3s linear; }

          .header{ padding:20px 32px; display:flex; align-items:center; }
          .logoWrap{ display:flex; align-items:center; gap:20px; border-radius: 20px; overflow: hidden; }
          .brandTitle { font-size: 26px; font-weight: 900; letter-spacing: -1.2px; color: #111; }
          .sub{ margin:0; margin-top:4px; font-size:10px; letter-spacing:6px; color:var(--accent); opacity: 0.7; font-weight: 800; text-transform: uppercase; }
          ...
          .nowPlayingArea{ padding: 20px 28px 24px; display: flex; align-items: center; justify-content: space-between; gap: 24px; }
          .titleContent { flex: 1; min-width: 0; }
          .nowLabel { font-size: 10px; font-weight: 900; color: var(--accent); letter-spacing: 2px; text-transform: uppercase; margin-bottom: 4px; }
          .nowPlayingTitle { font-size: 20px; font-weight: 850; letter-spacing: -0.5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #111; line-height: 1.2; }
          
          .playerControls { display: flex; align-items: center; gap: 22px; flex-shrink: 0; }
          .controlBtn { background: none; border: none; cursor: pointer; color: rgba(0,0,0,0.25); display: flex; align-items: center; transition: all 0.3s ease; }
          .controlBtn:hover { color: var(--accent); transform: scale(1.1); }
          .controlBtn.mainPlay svg { width: 32px; height: 32px; }
          .controlBtn.sideBtn svg { width: 22px; height: 22px; }
          
          .listHeaderArea{ padding: 18px 28px 14px; border-bottom: 1px solid rgba(0,0,0,0.03); }
          .listTitleRow{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
          .listMainTitle { font-size: 18px; font-weight: 900; letter-spacing: -0.5px; color: #111; }
          .listCounter { font-size: 11px; font-weight: 800; color: var(--accent); opacity: 0.6; }
          
          .toolbarInTitle{ display: flex; align-items: center; gap: 14px; }
          .iconBtn{ background: none; border: none; cursor: pointer; color: rgba(0,0,0,0.15); transition: all 0.3s ease; display: flex; align-items: center; }
          .iconBtn.active{ color: var(--accent); transform: scale(1.1); }
          .iconBtn svg { width: 15px; height: 15px; stroke-width: 2.5px; }

          .searchInside{ display:flex; align-items:center; gap:12px; padding:10px 18px; border-radius:16px; background:rgba(255,255,255,0.4); border: 1px solid rgba(0,0,0,0.04); }
          ...
          /* ✅ 列表项：修正缩略图排版与黑边 */
          .item{ display:grid; grid-template-columns: 32px 110px 1fr; gap:20px; padding:16px 28px; cursor:pointer; transition: all 0.4s ease; align-items:center; }
          .item:hover { background: rgba(255,255,255,0.4); transform: translateX(6px); }
          .item.active{ position:relative; background: transparent; }
          .item.active::before{ content:""; position:absolute; left:0; height:30%; width:4px; background:var(--accent); border-radius:0 10px 10px 0; }
          
          .trackIdx { font-size: 12px; font-weight: 800; color: rgba(0,0,0,0.1); font-variant-numeric: tabular-nums; }
          .itemThumb{ width:110px; min-width:110px; height:62px; border-radius:12px; overflow:hidden; background:rgba(0,0,0,0.03); flex-shrink: 0; }
          .itemThumb img{ width: 100%; height: 100%; object-fit: cover; }
          
          .itemTitle { font-size: 14px; font-weight: 750; line-height: 1.45; color: #374151; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
          .itemSub { font-size: 11px; color: rgba(0,0,0,0.3); font-weight: 600; margin-top: 4px; text-transform: uppercase; }

          @media (max-width:1024px){ 
            .page { padding: 12px 8px 30px; }
            .mainGrid { grid-template-columns: 1fr; } 
            .item { grid-template-columns: 24px 90px 1fr; padding: 12px 16px; gap: 14px; } 
            .itemThumb { width: 90px; min-width: 90px; height: 50px; }
          }
          .footer{ margin-top:20px; text-align:center; color:rgba(0,0,0,0.15); font-size:11px; font-weight: 700; letter-spacing: 1px; }
        `}</style>
        ...
        <div className="container" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mainGrid">
            <section className="glass-card glass-hero flex flex-col">
              ...
              <div className="progressBarContainer">
                <div className="progressBarActive" style={{ width: `${progress}%` }} />
              </div>
              ...
              {now && (
                <div className="nowPlayingArea">
                  ...
                  <div className="playerControls">
                    <button className="controlBtn sideBtn" onClick={(e) => { e.stopPropagation(); handlePlayPrev(); }}>
                      <svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg>
                    </button>
                    <button className="controlBtn mainPlay" onClick={togglePlay}>
                      {isPlaying ? (
                    ...
                    <button className="controlBtn sideBtn" onClick={(e) => { e.stopPropagation(); handlePlayNext(); }}>
                ...
                <div className="listTitleRow">
                  ...
                  <div className="toolbarInTitle">
                    ...
                    <button className={`iconBtn ${playMode === 'LIST_LOOP' ? 'active' : ''}`} onClick={() => setPlayMode('LIST_LOOP')}>
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path></svg>
                    </button>
                    <button className={`iconBtn ${playMode === 'SINGLE_LOOP' ? 'active' : ''}`} onClick={() => setPlayMode('SINGLE_LOOP')}>
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="m17 2 4 4-4 4"></path><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><path d="m7 22-4-4 4-4"></path><path d="M21 13v2a4 4 0 0 1-4 4H3"></path><path d="M11 10h1v4"></path></svg>
                    </button>
                  </div>
                </div>
                <div className="searchInside">
                  <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="搜索歌曲…" />
                </div>
              </div>
